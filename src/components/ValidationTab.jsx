...
export default function ValidationTab() {
  ...
  const [twoChildLimitData, setTwoChildLimitData] = useState([]);

  // Load Scotland baseline and two-child limit data
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load two-child limit data
        if (tclRes.ok) {
          const tclCsvText = await tclRes.text();
          const tclParsed = parseCSV(tclCsvText);
          const tclData = tclParsed.map((row) => ({
            year: parseInt(row.year),
            peAffectedChildren: parseInt(row.pe_affected_children),
            peCostMillions: parseFloat(row.pe_cost_millions),
            sfcAffectedChildren: parseInt(row.sfc_affected_children),
            sfcCostMillions: parseFloat(row.sfc_cost_millions),
          }));
          setTwoChildLimitData(tclData);
        }
    ...
    <div className="scotland-tab">
      ...
      <div className="section-box">
        <p className="chart-description">
          ...
          to Scotland. PolicyEngine estimates are generated by comparing UC entitlement with and without
          the two-child limit.
        </p>
        ...
        <div className="comparison-table-container">
          <table className="comparison-table">
            ...
            <tbody>
              {(() => {
                const tcl2026 = twoChildLimitData.find(d => d.year === 2026);
                const tcl2029 = twoChildLimitData.find(d => d.year === 2029);
                return (
                  <>
                    <tr>
                      <td className="metric-name">
                      ...
                      <td className="official-value">
                        <a href="https://fiscalcommission.scot/mitigating-the-two-child-limit-and-the-scottish-budget/" target="_blank" rel="noopener noreferrer">
                          £{tcl2026?.sfcCostMillions || 155}m
                        </a>
                      </td>
                      <td className="pe-value">
                        £{tcl2026?.peCostMillions || "—"}m
                      </td>
                      <td className="difference">
                        {tcl2026 ? formatDifference(tcl2026.peCostMillions, tcl2026.sfcCostMillions) : "—"}
                      </td>
                    </tr>
                    <tr>
                      <td className="metric-name">
                      ...
                      <td className="official-value">
                        <a href="https://fiscalcommission.scot/mitigating-the-two-child-limit-and-the-scottish-budget/" target="_blank" rel="noopener noreferrer">
                          £{tcl2029?.sfcCostMillions || 198}m
                        </a>
                      </td>
                      <td className="pe-value">
                        £{tcl2029?.peCostMillions || "—"}m
                      </td>
                      <td className="difference">
                        {tcl2029 ? formatDifference(tcl2029.peCostMillions, tcl2029.sfcCostMillions) : "—"}
                      </td>
                    </tr>
                    <tr>
                      <td className="metric-name">
                      ...
                      <td className="official-value">
                        <a href="https://fiscalcommission.scot/mitigating-the-two-child-limit-and-the-scottish-budget/" target="_blank" rel="noopener noreferrer">
                          {(tcl2026?.sfcAffectedChildren || 43000).toLocaleString()}
                        </a>
                      </td>
                      <td className="pe-value">
                        {tcl2026?.peAffectedChildren?.toLocaleString() || "—"}
                      </td>
                      <td className="difference">
                        {tcl2026 ? formatDifference(tcl2026.peAffectedChildren, tcl2026.sfcAffectedChildren) : "—"}
                      </td>
                    </tr>
                    <tr>
                      <td className="metric-name">
                      ...
                      <td className="official-value">
                        <a href="https://fiscalcommission.scot/mitigating-the-two-child-limit-and-the-scottish-budget/" target="_blank" rel="noopener noreferrer">
                          {(tcl2029?.sfcAffectedChildren || 50000).toLocaleString()}
                        </a>
                      </td>
                      <td className="pe-value">
                        {tcl2029?.peAffectedChildren?.toLocaleString() || "—"}
                      </td>
                      <td className="difference">
                        {tcl2029 ? formatDifference(tcl2029.peAffectedChildren, tcl2029.sfcAffectedChildren) : "—"}
                      </td>
                    </tr>
                  </>
                );
